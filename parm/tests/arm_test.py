...
class ArmTest(TestCase):
    ...
    def test_mov(self):
        expected = Block([Line(Instruction('mov', [Reg('r0'), Immediate(0x10)]))])
        assert self._pt('mov r0, #0x10') == expected

    def test_wrong_opcode(self):
        expected = Block([Line(Instruction('ldr', [Reg('r1'), MemAccessOffset(Reg('r1'))]))])
        assert self._pt('str r1, [r1]') != expected

    def test_same_address(self):
        expected = Block([Line(Instruction('mov', [Reg('r0'), Immediate(0x10)]), Address(0x300))])
        assert self._pt('0x300: mov r0, #0x10') == expected

    def test_different_address(self):
        unexpected = Block([Line(Instruction('mov', [Reg('r0'), Immediate(0x10)]), Address(0x400))])
        assert self._pt('0x300: mov r0, #0x10') != unexpected

    def test_ldm(self):
        expected = Block([Line(Instruction('ldm', [Reg('r0'), MemMulti(RegList([Reg('r0'), Reg('r2')]))]))])
    ...
    def test_ldm_range(self):
        expected = Block([Line(Instruction('ldm', [Reg('r0'), MemMulti(RegList([Reg('r0'), Reg('r1'), Reg('r2')]))]))])
        assert self._pt('LDM R0, {R0-R2}') == expected

    def test_conditional(self):
        inst = Instruction('mov', [Reg('r0'), ShiftedReg(Reg('r1'))])
        expected = Block([Line(inst)])
    ...
    def test_blx(self):
        expected = Block([Line(Instruction('blx', [Reg('r1')]))])
        assert self._pt('blx r1') == expected

    def test_bl(self):
        expected = Block([Line(Instruction('bl', [Address(0x2000)]))])
        assert self._pt('bl 0x2000') == expected

    def test_b(self):
        expected = Block([Line(Instruction('b', [Address(0x4000)]))])
    ...
    def test_scaled_sub(self):
        expected = Block([Line(Instruction('sub', [Reg('r0'), Reg('r1'), ShiftedReg(Reg('r2'), 'lsl#6')]))])
        assert self._pt('SUB r0, r1, r2, lsl#6') == expected

    def test_scaled_mov(self):
        expected = Block([Line(Instruction('mov', [Reg('r0'), ShiftedReg(Reg('r3'), 'lsl#30')]))])
        assert self._pt('mov r0, r3, lsl#30') == expected

    def test_bad_scaled_sub(self):
        expected = Block([Line(Instruction('sub', [Reg('r0'), Reg('r1'), ShiftedReg(Reg('r2'), 'lsl#7')]))])
        assert self._pt('SUB r0, r1, r2, lsl#6') != expected
    ...
    def test_ldm_interspersed_range(self):
        expected = Block([Line(Instruction('ldm', [Reg('r0'), MemMulti(
            RegList([Reg('r0'), Reg('r2'), Reg('r3'), Reg('r4'), Reg('r5'), Reg('lr'), Reg('pc')]))]),
                               Address(0x1000))])
